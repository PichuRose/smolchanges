...
public class CustomShapes {
    ...
    public static final VoxelShape LONG_ROD_SHAPE_4 = createCustomShape(6, 0, 6, 10, 16, 10);
    public static final VoxelShape MEDIUM_ROD_SHAPE = createCustomShape(7, 0, 7, 9, 11, 9);
    ...
    //public static final VoxelShape FENCE_SHAPE = subtract(LONG_ROD_SHAPE_4, createCustomShape());
    ...
    public static VoxelShape subtract(VoxelShape... shapes) {
        if (shapes.length == 0) return null;
        if (shapes.length == 1) return shapes[0];
        VoxelShape shape = shapes[0];
        for (int i = 1; i < shapes.length; i++) shape = Shapes.joinUnoptimized(shapes[i], shape, BooleanOp.ONLY_SECOND);
        //joinUnoptimized(VoxelShape pShape1, VoxelShape pShape2, BooleanOp pFunction)
        return shape;
    }
}
